...
export class Canvas
{
    ...
    public zoom(amount : number | Point, transformPrefix: string, transformPostfix: string)
    {
        ...
        this.canvas.style.transform = transformPrefix +
    ...
    public drawHexagon(position : Point, radius : number, color : string, lineWidth : number) : void
    {
        const a = 2 * Math.PI / 6;
        ...
        for (let i = 0; i < 6; i++)
        {
            this.context.lineTo(position.x + radius * Math.cos(a * i), position.y + radius * Math.sin(a * i));
        }
        this.context.closePath();
        this.context.stroke();
    }

    public drawHexagonCell(startPosition : Point, cellCoordinate : Point, radius : number, startHigh : boolean, color : string, lineWidth : number, label? : string, directions?: number[])
    {
        ...
        const startY = shouldStartHigh ? startPosition.y : startPosition.y + radius * (Math.sin(a));

        this.context.beginPath();
        for (let i = 0; i < 6; i++)
        {
            this.context.lineTo(
                startPosition.x + (radius * (1 + Math.cos(a)) * x) + (radius * Math.cos(a * i)) + 0.5, 
                startY + (radius * 2 * (Math.sin(a)) * y) + (radius * Math.sin(a * i)) + 0.5
            );
        }
        this.context.closePath();
        this.context.stroke();
        const centerPt = new Point(startPosition.x + (radius * (1 + Math.cos(a)) * x), startY + (radius * 2 * (Math.sin(a)) * y) + 5);
        ...
        if (directions)
        {
            directions.forEach((direction) =>
            {
                ...
                this.drawRotatedImage(
                    ...
                    centerPt
                        .minus(new Point(0, 5))
                        .plus(new Point(Math.cos(angle), Math.sin(angle)).times(radius - 16))
    ...
    public fillHexagonCell(startPosition : Point, cellCoordinate : Point, radius : number, startHigh : boolean, color : string)
    {
        const { x, y } = cellCoordinate;
        const a = 2 * Math.PI / 6;
    ...
    public drawHexagonGrid(startPosition : Point, radius : number, gridDims : Point, startHigh : boolean, color : string, lineWidth : number, labels? : string[], directions? : number[][]) : Point[]
    {
        const a = 2 * Math.PI / 6;
