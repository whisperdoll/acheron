...
const WormholeToken: TokenDefinition<Store> = {
  ...
  callbacks: {
    ...
    onTick(store, helpers, playheads) {
      ...
      function tryPerformWormhole(playheadIndex: number) {
        if (probability / 100 > Math.random()) {
          function canSendTo(other: any) {
            const weCanSend =
              (syncChannels ? channel : sendToChannel) === other.channel;
            const theyCanReceive =
              (other.syncChannels
                ? other.channel
                : other.receiveFromChannel) === channel;
            return weCanSend && theyCanReceive;
          }

          const ph = playheads[playheadIndex];

          const wormholes = helpers
            .getOtherTokenInstances()
            .filter((w: any) => canSendTo(w))
          ...
          if (wormholes.length > 0) {
            ...
            helpers.warpPlayhead(
              playheadIndex,
              wormholes[gotoIndex].hexIndex,
              wormholes[gotoIndex].layerIndex
            );
